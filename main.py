...
class ActionStack:
    ...
    def push(self, action_type, expense):
...
class PrefixSumManager:  # префиксные суммф для обработки запросов суммы за период
    ...
    def rebuild_from_expenses(
        self, expenses: List[Expense]
    ):  # обновляет массив префиксных сумм, когда нужно пересчитать всё
        if not expenses:
    ...
    def incremental_add(
    ...
    ):  # обновление префиксных сумм при добавлении расхода
        ...
        if day in self.prefix_sums:
            ...
            prev_day_before = (
                self.prefix_sums.get(day - 1, 0.0) if day - 1 >= 0 else 0.0
            )
            prev_total_for_day = prev_day_total - prev_day_before
        new_total_for_day = (
            prev_total_for_day + expense.amount
        )  # Вычисляем сумму за новый день
        ...
        if day not in self.prefix_sums:  # Обновляем префиксные суммы
            ...
            prev_cumulative = (
                self.prefix_sums.get(self.sorted_days[idx - 1], 0.0) if idx > 0 else 0.0
            )
    ...
    def incremental_remove(self, expense: Expense):  # удаление расхода
        ...
        if day not in self.prefix_sums:
            return
        for d in self.sorted_days[
            self.sorted_days.index(day) :
        ]:  # новые префиксные суммы для этого дня и всех других
            self.prefix_sums[d] -= expense.amount
        prev_cumulative = (
            self.prefix_sums.get(day - 1, 0.0) if day - 1 in self.prefix_sums else 0.0
        )
        if (
            self.prefix_sums[day] == prev_cumulative
        ):  # Если сумма за день после удаления расхода стала равна 0 то удаляем этот день
            del self.prefix_sums[day]
            self.sorted_days.remove(day)

    def range_sum(
        self, day_a: int, day_b: int
    ) -> float:  # Сумма расходов за период с day_a по day_b
        if (
            day_a > day_b
        ):  # если вдруг начальный день больше конечного, то меняем их местами
            day_a, day_b = day_b, day_a
        sum_up_to_b = self._get_prefix_sum(day_b)
        sum_up_to_a_minus_1 = self._get_prefix_sum(
            day_a - 1
        )  # префиксные суммы для дня day_b и дня day_a-1
        return sum_up_to_b - sum_up_to_a_minus_1

    def _get_prefix_sum(
        self, day: int
    ) -> float:  # получение суммы расходов с первого дня по указанный день включительно
        ...
        if idx >= 0:
            return self.prefix_sums[
                self.sorted_days[idx]
            ]  # если запрашиваемого дня нет - ищем предыдущий день и его префиксную сумму расходов
        return 0.0
...
def find_day_with_max_expense(
    tree_root: Optional[BSTtree],
) -> Tuple[Optional[int], float]:  # поиск дня с максимальным расходом
...
def insertion_sort_categories_by_total(
    categories: dict,
) -> List[Tuple[str, float]]:  # Сортировка категорий по сумме трат
    if not categories:
        return []
    category_totals = []
...
def get_top_n_categories(
    categories: dict, n: int = 3
) -> List[Tuple[str, float]]:  # получения топа категорий по тратам
    sorted_cats = insertion_sort_categories_by_total(categories)
    return sorted_cats[:n]


class ExpenseTrackerWithAlgorithms(
    ExpenseTracker
):  # расширенный класс с добавлением новых методов
    def __init__(self):
        super().__init__()
        self._prefix_manager = PrefixSumManager()

    def add_expense(
        self, day: int, category_name: str, amount: float
    ):  # добавление расхода с обновлением других сумм
        expense = Expense(day, amount, category_name)
        category_data = self.get_category(category_name)
        category_data["history"].append(
            (day, amount, expense)
        )  # добавляем то на что потратили деньги в категорию
        if day in category_data["day_totals"]:
            category_data["day_totals"][day] += amount
        else:
            category_data["day_totals"][day] = amount
    ...
    def undo(self):  # отмена последнего действия с последующим обновлением других сумм
        ...
        if action_type == "ADD":
            if (
                self.tree_root
            ):  # удаляем из дерева, истории категорий, обнавляем префиксные суммы
                self.tree_root.remove_expense(expense)
            category_data = self.categories[expense.category]
            for i, record in enumerate(category_data["history"]):
                if record[2] == expense:
    ...
    def get_range_sum(
        self, day_a: int, day_b: int
    ) -> float:  # Сумма расходов за определённый период
        return self._prefix_manager.range_sum(day_a, day_b)

    def get_day_with_max_expense(
        self,
    ) -> Tuple[Optional[int], float]:  # вычисление дня с максимальным расходом
    ...
    def get_sorted_categories(
        ...
        Tuple[str, float]
    ]:  # вернуть категории, отсортированные по сумме трат по убыванию
        return insertion_sort_categories_by_total(self.categories)

    def get_top_categories(
        self, n: int = 3
    ) -> List[Tuple[str, float]]:  # вернуть топ категорий по тратам
...
def calc_forecast(
    tracker: ExpenseTrackerWithAlgorithms,
    current_day: int,
    ...
    )  # сколько всего потрачено сейчас, берём готовую сумму у участника 2

    if current_day <= 0:
        return {"error": "Текущий день должен быть положительным"}

    avg_per_day = total_so_far / current_day  # среднее количество трат в день
    forecast_total = avg_per_day * days_in_month  # прогноз на основе текущих трат
    ...
    if budget_limit is not None:
        ...
        )  # True, если прогноз больше лимита
        result["difference"] = forecast_total - budget_limit  # на сколько превышен
...
class BudgetApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.tracker = (
            ExpenseTrackerWithAlgorithms()
        )  # хранит все расходы, дерево, стек и префиксные суммы
    ...
    def add_box(self):
        ...
        # поле "категория"
        ttk.Label(frame, text="Категория:").grid(
        ...
        # поле "сумма"
        ttk.Label(frame, text="Сумма:").grid(
    ...
    def act_box(self):
        ...
        ttk.Label(frame, text="Текущий день месяца:").grid(
        ...
        ttk.Label(frame, text="Лимит бюджета (необязательно):").grid(
    ...
    def add(self):
        try:
            ...
            messagebox.showerror(
                "Ошибка", "Проверьте поля: день и сумма должны быть числами"
            )
            return
    ...
    def calc(self):
        ...
        total = self.tracker.get_range_sum(
            a, b
        )  # сам расчёт через префиксные суммы участника 2
    ...
    def undo(self):
        expense = (
            self.tracker.undo()
        )  # достаёт последнее действие из стека и откатывает его
        if expense is None:
